#!/usr/bin/env node
// ============================================================
// GROUNDWORK STUDIOS — TIER 1 "FOUNDATION" BUILD
// Same business, same config, same design system as the Tier 2
// build.js — but only the base 10 pages, no booking/calculators/
// visualiser/chat/blog/case-studies/team/areas/guarantee/finance/
// social/dark-mode/design-menu. Before/after is static, not a
// drag slider. Contact is a single plain form.
// Outputs: dist-tier1/
// ============================================================
...
const DIST = path.join(ROOT, "dist-tier1");

// ── helpers (same as build.js) ────────────────────────────
...
const services = (c.SERVICES || []).map(s => ({ ...s, slug: s.slug || slug(s.name) }));
...
// ── SVG icons (same set as build.js) ──────────────────────
...
// Simple nav — no "More" dropdown, since none of those pages exist here.
function buildHeader() {
  return `
<div id="announcement-bar-slot"></div>
${buildUtilityBar()}
...
<div id="mobile-sticky" role="navigation" aria-label="Quick actions">
  <a href="tel:${esc(c.PHONE_HREF)}" class="mob-call">${svg("phone", 20)} Call Us</a>
  <a href="/contact/" class="mob-quote">${svg("msg", 20)} Free Quote</a>
</div>`;
}
...
// Only main.js — no widgets.js (that's all Tier 2 interactivity).
...
// ── Static before/after pair — no drag slider ──────────────
...
<section id="ba-teaser" aria-labelledby="teaser-heading">
  <div class="container">
    ...
    <p class="section-intro">Real jobs, real results.</p>
    <div class="teaser-grid">
      ${items.map(p => `
      <div class="teaser-card">
        ${baPair(p.beforeImage, p.afterImage, "Before — " + (p.title || ""), "After — " + (p.title || ""))}
        <div class="teaser-caption">
          <span>${esc(p.title || c.SERVICE_AREA)}</span>
...
<section id="services" aria-labelledby="services-heading">
  <div class="container">
    ...
    <div class="services-grid">${cards.join("")}</div>
  </div>
</section>`;
}
...
<section id="why-us" aria-labelledby="why-heading">
  <div class="container">
    ...
    <div class="usps-grid">
      <div class="usp-item"><div class="usp-icon">${svg("check", 20)}</div><div><h3>Free, No-Obligation Quotes</h3><p>We visit, assess and give you a clear written quote. No pressure, no hidden costs.</p></div></div>
      <div class="usp-item"><div class="usp-icon">${svg("shield", 20)}</div><div><h3>Fully Insured &amp; Guaranteed</h3><p>All work is backed by public liability insurance and our workmanship guarantee.</p></div></div>
      <div class="usp-item"><div class="usp-icon">${svg("clock", 20)}</div><div><h3>${esc(c.YEARS_TRADING)} Years Local Experience</h3><p>Trusted by homeowners across ${esc(c.SERVICE_AREA)} for over ${esc(c.YEARS_TRADING)} years.</p></div></div>
      <div class="usp-item"><div class="usp-icon">${svg("users", 20)}</div><div><h3>No Subcontractors</h3><p>Your job is done by our own skilled team — same quality every time.</p></div></div>
      <div class="usp-item"><div class="usp-icon">${svg("dollar", 20)}</div><div><h3>Clear, Honest Pricing</h3><p>The price we quote is the price you pay. Every cost explained upfront.</p></div></div>
      <div class="usp-item"><div class="usp-icon">${svg("msg", 20)}</div><div><h3>Easy to Reach</h3><p>We answer calls and messages quickly. You'll always know where you stand.</p></div></div>
    </div>
  </div>
</section>`;
}
...
function buildTestimonialsSection(items, limit = 3) {
  ...
  <div class="rating-band">
    ${c.RATING_TEXT ? `<div class="rating-band-score"><div class="big-score">5.0</div><div class="score-stars">★★★★★</div><div class="score-label">${esc(c.RATING_TEXT)}</div></div>` : ""}
    ${c.RATING_TEXT && c.CUSTOMER_COUNT ? `<div class="rating-band-sep"></div>` : ""}
    ${c.CUSTOMER_COUNT ? `<div class="rating-band-score"><div class="big-score">${esc(c.CUSTOMER_COUNT.replace(/[^\d+]/g, ""))}+</div><div class="score-label">Happy Customers</div></div>` : ""}
  </div>` : "";
  return `
...
<section id="faq" aria-labelledby="faq-heading">
  <div class="container">
    ...
    <div class="faq-list">
      ${faqList.map(f => `
      <div class="faq-item">
        <button class="faq-question" aria-expanded="false">${esc(f.q)}<span class="faq-icon" aria-hidden="true">+</span></button>
...
// ── Portfolio (static pairs, no slider) ────────────────────

function buildProjectCard(p) {
  const cat = p.category || p.tag || "";
  return `
  <div class="project-card" data-category="${esc(cat)}">
    ${baPair(p.beforeImage, p.afterImage, "Before — " + (p.title || ""), "After — " + (p.title || ""))}
...
function buildHomePage() {
  ...
  return buildHead({ title: c.META_TITLE, description: c.META_DESCRIPTION, canonical: "/", extraSchema: schema }) + `
  ...
  <section id="hero" role="main">
    ...
    <div class="hero-inner">
      <div class="hero-content">
        ...
        <div class="hero-trust">${buildHeroTrustChips()}</div>
      </div>
      ${buildHeroMiniForm()}
    </div>
  </section>
...
function buildPortfolioPage() {
  ...
  return buildHead({
    ...
    canonical: "/portfolio/", extraSchema: schema,
  }) + `
...
<main>
  <div class="portfolio-hero">
    <div class="container">
      ...
      <p>Real transformations across ${esc(c.SERVICE_AREA)}.</p>
      <div class="portfolio-stats">
        ${c.PROJECT_COUNT ? `<div class="portfolio-stat"><div class="pstat-num">${esc(String(c.PROJECT_COUNT))}+</div><div class="pstat-label">Projects Completed</div></div>` : ""}
  ...
  <section style="background:var(--surface);">
    <div class="container">
      ...
      <div class="project-grid" id="project-grid">
        ${portfolio.map(p => buildProjectCard(p)).join("")}
      </div>
    </div>
  </section>
...
function buildServicePage(service) {
  ...
  return buildHead({
    title: `${service.name} in ${c.SERVICE_AREA} — ${c.BUSINESS_NAME}`,
    description: (service.longDescription ? service.longDescription.slice(0, 155) + "…" : `Expert ${service.name.toLowerCase()} services in ${c.SERVICE_AREA}. Free quotes, fully insured. Call ${c.PHONE}.`),
    canonical: `/services/${service.slug}/`, extraSchema: schema,
  }) + `
...
<main>
  <div class="service-page-hero">
    <div class="container">
      <div class="breadcrumb"><a href="/">Home</a> / <a href="/services/${esc(service.slug)}/">${esc(service.name)}</a></div>
    ...
    <div class="container">
      <div class="service-page-grid">
        <div class="service-page-content">
          ...
          <p>${esc(service.longDescription || service.description || "")}</p>
...
function buildAboutPage() {
  ...
  return buildHead({
    ...
    canonical: "/about/", extraSchema: schema,
  }) + `
      ...
      <div class="about-grid">
        ...
        <div class="about-text">
          ...
          <div class="about-stats">
            <div class="stat-item"><div class="stat-num">${esc(c.YEARS_TRADING)}+</div><div class="stat-label">Years Experience</div></div>
            ${c.CUSTOMER_COUNT ? `<div class="stat-item"><div class="stat-num">${esc(c.CUSTOMER_COUNT.replace(/[^\d+]/g, ""))}+</div><div class="stat-label">Happy Customers</div></div>` : ""}
            ${c.RATING_TEXT ? `<div class="stat-item"><div class="stat-num">5★</div><div class="stat-label">Average Rating</div></div>` : ""}
          </div>
...
function buildReviewsPage() {
  ...
  return buildHead({
    ...
    canonical: "/reviews/", extraSchema: schema,
  }) + `
...
function buildContactPage() {
  ...
  return buildHead({
    ...
    canonical: "/contact/", extraSchema: schema,
  }) + `
      ...
      <div class="contact-grid">
        <div class="contact-form-wrap">
          ...
          <form id="contact-form" novalidate>
            ...
            <input type="hidden" name="from_name"   value="${esc(c.BUSINESS_NAME)} Website" />
            <input type="checkbox" name="botcheck" style="display:none" />
            <div class="form-group"><label for="f-name">Your Name *</label><input type="text" id="f-name" name="name" required placeholder="John Smith" /></div>
            <div class="form-group"><label for="f-phone">Phone Number *</label><input type="tel" id="f-phone" name="phone" required placeholder="07700 900000" /></div>
            <div class="form-group"><label for="f-email">Email Address</label><input type="email" id="f-email" name="email" placeholder="john@example.com" /></div>
            <div class="form-group">
            ...
            <div class="form-group"><label for="f-message">Tell us about your project</label><textarea id="f-message" name="message" placeholder="e.g. Double driveway, approx 50m²…"></textarea></div>
            <button type="submit" class="btn btn-primary form-submit">Send Enquiry</button>
        ...
        <div class="contact-info-wrap">
          <div class="contact-info-card">
            ...
            <div class="contact-detail"><div class="contact-detail-icon">${svg("phone", 20)}</div><div><h4>Phone</h4><a href="tel:${esc(c.PHONE_HREF)}">${esc(c.PHONE)}</a></div></div>
            <div class="contact-detail"><div class="contact-detail-icon">${svg("mail", 20)}</div><div><h4>Email</h4><a href="mailto:${esc(c.EMAIL)}">${esc(c.EMAIL)}</a></div></div>
            <div class="contact-detail"><div class="contact-detail-icon">${svg("pin", 20)}</div><div><h4>Service Area</h4><p style="margin:0;color:var(--ink)">${esc(c.SERVICE_AREA)}</p></div></div>
          </div>
          ${hoursRows ? `<div class="contact-info-card"><h3>Opening Hours</h3><ul class="hours-list">${hoursRows}</ul></div>` : ""}
        </div>
      </div>
    </div>
  </section>
...
function buildSitemap() {
  ...
  const urls = pages.map(p => `  <url><loc>${p.loc}</loc><lastmod>${today}</lastmod><priority>${p.priority}</priority></url>`).join("\n");
...
// Only style.css + main.js — no showcase.css / widgets.js.
fs.mkdirSync(path.join(DIST, "src/css"), { recursive: true });
fs.mkdirSync(path.join(DIST, "src/js"), { recursive: true });
fs.copyFileSync(path.join(ROOT, "src/css/style.css"), path.join(DIST, "src/css/style.css"));
fs.copyFileSync(path.join(ROOT, "src/js/main.js"),    path.join(DIST, "src/js/main.js"));
copyDir(path.join(ROOT, "images"), path.join(DIST, "images"));

console.log("✅ Tier 1 build complete →");
console.log(`   Home:      dist-tier1/index.html`);
console.log(`   Pages:     portfolio, about, reviews, contact`);
